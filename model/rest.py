...
class ResT(nn.Module):  # this model works with any image size, even non-square image size
    def __init__(self, model_name: str = 'S', pretrained: str = None, num_classes: int = 5, *args, **kwargs) -> None:
        super().__init__()
        ...
        self.stem = Stem(7, embed_dims[0])   #in channel
...
if __name__ == '__main__':
    model = ResT('S', pretrained=None, num_classes=5)
    x = torch.zeros(1, 7, 1024, 256)
    y = model(x)
    print(y.shape)
