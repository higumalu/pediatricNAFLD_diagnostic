...
class PatchEmbedOverlap(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0, embed_dim=768, norm_layer=nn.BatchNorm2d, use_norm=True):
        super().__init__()
        self.proj = nn.Conv2d(7, embed_dim, patch_size, stride, padding)    #in_channel
...
if __name__ == '__main__':
    model = WaveMLP(model_name = 'T', pretrained = None, num_classes = 5) 
    #pretrained weight in author github
    x = torch.randn(1, 7, 256, 1024)
    y = model(x)
    print(y.shape)
